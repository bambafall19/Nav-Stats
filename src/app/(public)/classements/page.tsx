                      ...
                      <div style={{ background: 'var(--color-surface-card)', border: '1px solid var(--color-border)', borderRadius: 'clamp(12px, 3vw, 16px)', overflow: 'hidden' }}>
                        {eqs.map((eq: any, i: number) => (
                          <div key={eq?.id || i} style={{
                            ...
                            {eq?.logo_url ? (
                              <img src={eq.logo_url} alt={eq?.nom || 'Équipe'} style={{ width: 'clamp(28px, 6vw, 32px)', height: 'clamp(28px, 6vw, 32px)', borderRadius: 6, objectFit: 'cover', flexShrink: 0 }} />
                            ) : (
                              <div style={{
            ...
            <div style={{ marginBottom: 'clamp(24px, 5vw, 32px)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 'clamp(8px, 2vw, 12px)' }}>
                {[top3[1], top3[0], top3[2]].map((u, podiumIndex) => {
                  ...
                  return (
                    <Link key={u?.id || podiumIndex} href={`/profil/${u?.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                      <div style={{
          ...
          <div style={{ marginBottom: 'clamp(24px, 5vw, 32px)' }}>
            ...
            <div style={{ background: 'var(--color-surface-card)', border: '1px solid var(--color-border)', borderRadius: 'clamp(12px, 3vw, 16px)', overflow: 'hidden' }}>
              {rest.length > 0 ? rest.map((u, idx) => {
                const i = idx + 3
                const pct = (u?.total_pronostics || 0) > 0 ? Math.round(((u?.pronostics_corrects || 0) / u.total_pronostics) * 100) : 0
                return (
                  <Link key={u?.id || idx} href={`/profil/${u?.id}`} style={{ textDecoration: 'none', color: 'inherit', display: 'block' }}>
                    <div style={{
                      display: 'flex', alignItems: 'center', gap: 'clamp(8px, 2vw, 12px)',
                      padding: 'clamp(10px, 2vw, 14px) clamp(12px, 3vw, 16px)',
                ...
                {classementASC.map((a, i) => {
                  ...
                  return (
                    <div key={a?.asc || i} style={{
                      ...
                      {eq?.logo_url ? (
                        <img src={eq.logo_url} alt={a?.asc || 'ASC'} style={{ width: 'clamp(32px, 7vw, 36px)', height: 'clamp(32px, 7vw, 36px)', borderRadius: 8, objectFit: 'cover', flexShrink: 0 }} />
                      ) : (
                        <div style={{
                          width: 'clamp(32px, 7vw, 36px)', height: 'clamp(32px, 7vw, 36px)', borderRadius: 8, flexShrink: 0,
                          background: eq ? `linear-gradient(135deg, ${eq.couleur_principale || '#006233'}, ${eq.couleur_secondaire || '#00A651'})` : 'var(--color-surface)',
                          display: 'flex', alignItems: 'center', justifyContent: 'center',
